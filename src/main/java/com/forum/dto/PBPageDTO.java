...
public class PBPageDTO {
	...
	private int total;
	private PBCriteria pcri;

	public PBPageDTO(int total, PBCriteria pcri) {
		this.total=total;
		this.pcri=pcri;
		
//	    ex>3페이지=3/10->0.3 -> 1 * 10 = 10(끝페이지)
//	    ex>11페이지=11/10->1.1 -> 2 * 10 = 20(끝페이지)
		this.endpage = (int)(Math.ceil(pcri.getPpageNum() / 10.0)) * 10;
...
//	    ex>total: 300, 현재 페이지: 3 -> endPage: 10 => 300*1.0 / 10 => 30페이지
		int realEnd = (int)(Math.ceil((total * 1.0) / pcri.getPamount()));
